...
class BaseROPRidgeDataset(torch.utils.data.Dataset):
    ...
    def parse_input_list(self, root_dataset,annotation_folder,anno_filename, max_sample=-1, start_idx=-1, end_idx=-1):
        ...
        print('# samples: {}'.format(self.num_sample))
        '''
        self.cocoAnno = COCO(os.path.join(root_dataset,annotation_folder,anno_filename))
        self.list_sample = self.cocoAnno.getImgIds()
...
class TrainROPRidgeDataset(BaseROPRidgeDataset):
    def __init__(self, root_dataset_,opt_,img_folder_="train2014",annotation_folder_ = "annotations",
        ...
        super(BaseROPRidgeDataset, self).__init__(root_dataset = root_dataset_,
                opt=opt_,img_folder=img_folder_,annotation_folder=annotation_folder_ ,
    ...
    def _get_sub_batch(self):
        while True:
            ...
            sample_id= self.list_sample[self.cur_idx]
            this_sample = self.cocoAnno.loadImgs([sample_id])[0]
            if this_sample['height'] > this_sample['width']:
                self.batch_record_list[0].append(this_sample) # h > w, go to 1st class
            else:
                self.batch_record_list[1].append(this_sample) # h <= w, go to 2nd class
